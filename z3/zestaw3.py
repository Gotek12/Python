...
if (x > y):
    ...
    result = y;
# taki kod zadziała ale według pep8 zamiast x > y należy używać x>y
# również dopuszcza nawiasy ale dla tak krótkiego warunku są one niepotrzebne
# średników też nie trzeba stosować ale jeśli chcielibyśmy zrboić tak x = 2; y = 2 to należy je stosować aby kod był bardziej zrozumiały
...
# lepiej w przykładzie poniżej

# c
# for i in "axby": print ord(i) if ord(i) < 100 else i
# dla pythona3 brak () w print a takto składniowo można tak napisać funkcje ale jest mniej czytelna niż rozbicie jej na wiele linii
...
# X = 1, 2, 3; X[1] = 4
# x = 1,2,3 onstrukcja tworzy krotkę(tuple)  więc operacja x[1] jest niemożliwa bo krotki się nie zmienia
...
# jeśłi chcemy korzystać z range należy go przekonwertować na listę list(range(8))
# ale znów będzie problem gdyż będziemy mieli operacje int i lista

# jak dobrze rozumiem x to zmienna typu int więc
# 3.3
print("\n3.3")
for i in range(0, 31):
    if i % 3 != 0:
